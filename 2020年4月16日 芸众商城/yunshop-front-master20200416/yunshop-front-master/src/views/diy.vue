<template>
  <div id="all">
    <div class="search_nav"
        ...
        border: sliderNav.params.showborder == 1
      ...
      <div class="nav_box">
        <ul class="nav">
          <li v-for="item in sliderNav.menus" :key="item.title"
              @click="gotoAdvs(item.url)">
    ...
    <div style="height: 5.2625rem;"
         v-if="sliderNav.isshow ? sliderNav.isshow : false"></div>
    <div class="temp"
         v-if="temp.item">
      <div class="home"
           v-if="temp.item.pageinfo">
        <template v-if="temp.item.pageinfo.temp == 'topbar'">
          <!-- 浮动按钮 -->
          <div class="fe-floatico"
              ...
              top: temp.item.pageinfo.params.floattop
               ...
               :class="{
              'fe-floatico-right':
                temp.item.pageinfo.params.floatstyle == 'right'
            }">
            <a :href="temp.item.pageinfo.params.floathref">
              <img :src="temp.item.pageinfo.params.floatimg"
                   style="height:100%; width: 100%;" />
            </a>
          </div>
        </template>
        <div class="home"
             v-if="temp.item.data && !isInHome">
