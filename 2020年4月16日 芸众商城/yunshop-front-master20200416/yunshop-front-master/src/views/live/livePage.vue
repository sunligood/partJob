...
<template>
  <div id="livePage">
    <div class="videos" v-show="true" id="videoPlayer">
      <!-- <video-player class="vjs-custom-skin" ref="videoPlayer" :playsinline="true"  :options="playerOptions"  @ready="onPlayerReadied" @timeupdate="onTimeupdate"></video-player> -->
      <!-- <video-player class="video-player vjs-custom-skin" ref="videoPlayer" :x5-video-player-type="h5" :x5-video-player-fullscreen="true" :playsinline="true"  :options="playerOptions" style="object-fit:fill" @play="onPlayerPlay($event)" @pause="onPlayerPause($event)"></video-player> -->
    </div>
      ...
      <swiper-slide v-for="(item, index) in liveList" :key='item.id'>
        ...
        <div class="content" v-show="item.show">
          <div class="top" :style="{top:(fun.getPhoneEnv() == 2?(isPlaying?'2.625rem':'0.25rem'):'0.25rem')}">
            <div class="anchorInfo" @click="anchorPopup">
              ...
              <div class="user">
                ...
                <p>{{dataInfo.address}}| {{view_num}}观看</p>
              </div>
              <div class="attention" v-if="dataInfo.is_concern==0" @click.stop="addConcern">关注</div>
            </div>
          </div>
          <div class="directSeeding_content_center" v-if="dataInfo.banner" >
            <img :src="dataInfo.banner" alt="" style="width:100%;height:100%;">
            <!-- <div class="title">
              店铺优惠
            </div>
            <div class="preferential_list" @touchmove.stop>
              <div class="li" v-for="(item, index) in dataInfo.shop_discount" :key="index">满{{item.enough}}-{{item.reduce}}</div>
            </div> -->
          </div>
          <div class="positionStyles" :style="{bottom:(fun.getPhoneEnv() == 2?(isPlaying?'0.875rem':'2rem'):'2.125rem')}">
            ...
            <div style="display:flex;justify-content:space-between;">
              <div class="comment" @touchmove.stop>
                <ul class="chat_messages" id="chat_messages"   @touchmove.stop>
                  ...
                  <li class="chat_messages_child" v-for="(item, index) in messageList" :key="index" @click.stop>
                    <div class="text"><div class="message_icon" :class="[item.memberLevel>10?'':`color_${item.memberLevel}`] "><i class="iconfont icon-collect-active"></i></div><em class="name" :class="[item.memberLevel>10?'':`textcolor_${item.memberLevel}`] ">{{item.nickName}}</em>{{item.text}}</div>
                  </li>
                </ul>
              </div>
              <div class="top_icon">
                <div class="top_icons" @click="showPosterMethob" style="margin-bottom:0.625rem;"><i class="iconfont icon-bc_card"></i></div>
                <div class="top_icons" @click="shareMessage" style="margin-bottom:0.625rem;"><i class="iconfont icon-zb_share"></i></div>
                <div class="top_icons" @click="showReportPopup=true" ><i class="iconfont icon-zb_jubao"></i></div>
              </div>
            </div>
            <!-- 进度条 开始 -->
            ...
            <!--进度条 end -->
            <div class="bottomBox">
              <div class="commentLeft" @click="subMainCommentBtn"><i class="iconfont icon-liaotian"></i>{{commentMessage}}</div>
              <div class="bottomRight">
                ...
                <div class="top_icons journal-reward"  @click="addClickLove"><i class="iconfont icon-zb_like"></i><div class="num likenum" v-if="dataInfo.like_num">{{dataInfo.like_num}}</div></div>
    ...
    <!-- <van-popup v-model="mainCommentShow"
               position="bottom" @click-overlay="clickMessageOverlay" >
    ...
    <van-popup v-model="showUser" round closeable>
      <div class="popupClass1">
        ...
        <div class="bottom">
          <div class="flexbox">
            <div class="num">{{MemberInfo.view_count}}</div>
    ...
    <div class="popupClass_newPoster" @touchmove.stop v-if="!fun.isTextEmpty(PosterData)" :style="{left:(showPoster?'50%':'-150%')}">
            ...
            <div class="posterBottom">
              <div class="l_info">
                ...
                <div class="share_meber">
                  <img :src="PosterData.m_avatar" alt=""><div class="m_nickname">{{PosterData.m_nickname}}</div>的分享
                </div>
              </div>
              <div class="r_info">
        ...
        <div style="width: 19.375rem;height: 27.625rem;margin: 0 auto;border-radius: 8px;" v-show="!loadingImg">
          <img style="width: 19.375rem;height: 27.625rem;border-radius: 8px;z-index:2010;" id="thecanvas"/>
        </div>
      ...
      <div class="selectExchange">
        <div class="se-left">
          <van-radio-group v-model="radio">
            <van-radio name="1" checked-color="#ff3902" v-if="giftList.member.credit1">我当前{{giftList.member.credit1_name}}数量{{giftList.member.credit1?giftList.member.credit1:''}}</van-radio>
            <van-radio name="2" checked-color="#ff3902" v-if="giftList.member.credit2">我当前{{giftList.member.credit2_name}}{{giftList.member.credit2?giftList.member.credit2:''}}</van-radio>
            <van-radio name="3" checked-color="#ff3902" v-if="giftList.member.love">我当前{{giftList.member.love_name}}数量{{giftList.member.love?giftList.member.love:''}}</van-radio>
          </van-radio-group>
        </div>
            ...
            <div style="max-height:18rem;overflow-y:scroll;">
              <div class="activeList">
                ...
                <div class="activeChild" :class="items.activityData.packetBonusStatus==3?'failClass':'succedClass'" v-if="items.activityData.packetBonus">
                  ...
                  <div class="btn" v-if="items.activityData.packetBonusStatus==2" @click="gobalance">查看</div>
