<template>
  <div id="foreshow" :style="{height: innerHeight+ 'px'}">
    <div class="header"
         :style="{backgroundImage: 'url(' + PosterData.cover + ')'}">
    </div>
    <div class="title">
    ...
    <div class="content" :style="{width: fun.getPhoneEnv() == 3 ? '375px': ''}">
      ...
      <div class="number_time">
        ...
        <p class="active_time" v-show="!stop&&end">
          <span>{{ Day }}</span><em>天</em> <span>{{ Hour }}</span><em>:</em> <span>{{ Min }}</span><em>:</em><span>{{ Sec }}</span>
        </p>
    ...
    <!--<van-popup v-model="show" round style="background-color: transparent">-->
      <div class="popupClass2" @touchmove.stop :style="{top:(show?'50%':'-100%')}">
        ...
        <div style="width: 19.375rem;height: 27.625rem;margin: 0 auto;border: 1px solid #ffffff;border-radius: 8px;" v-show="!loadingImg">
          <img style="width: 19.375rem;height: 27.625rem;border-radius: 8px" id="thecanvas"/>
        </div>
