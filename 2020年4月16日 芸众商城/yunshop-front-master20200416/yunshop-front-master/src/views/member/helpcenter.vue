...
export default {
  ...
  methods: {
    ...
    share(data) {
      ...
      wx.ready(function() {
        let _title = that.sharedata.title
          ? that.sharedata.title
          : data.shop.name;
        let _link = document.location.href + "&share_tag=2";
        _link = that.fun.isMid(_link, data.info.uid);
        let _imgUrl = that.sharedata.icon
          ? that.sharedata.icon
          : data.shop.logo;
        let _desc = that.sharedata.description
          ? that.sharedata.description
          : data.shop.copyright;
        wx.showOptionMenu();
