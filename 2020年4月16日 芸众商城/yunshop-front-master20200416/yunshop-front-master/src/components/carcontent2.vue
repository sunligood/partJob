...
<template>
  <div id="carcontent2">
    <div class="branch"
         v-if="list.goo&&list.set.cat_adv_img"><a :href="list.set.cat_adv_url|href_filters"><img style="width:100%;"
    ...
    <div class="catagoryBox">
      <van-collapse v-model="activeName">
        <van-collapse-item title="标题1"
                           name="1">
          <dl>
            <dt v-for="(Secondary,index) in list"
                :key="index"
                :class="Secondary.id == secondaryId?'addClass' : 'removeClass'"
                @click.stop="chooseSecondary(Secondary)">{{Secondary.name}}</dt>
          </dl>
          <template v-if="list.set.cat_level=='3'&&!fun.isTextEmpty(threeInfo)">
            ...
            <dl>
              <dt v-for="(three,indexs) in threeInfo"
                  ...
                  @click="chooseThree(three.id)">{{three.name}}</dt>
            </dl>
          </template>
    ...
    <div class="goodsList"
      ...
      <div>
        <div class="goodWrapper">
          <ul class="list"
              v-for="(good,indexs) in list.goods_list"
              :key="indexs">
            <div class="img">
              <img :src="good.thumb" />
            </div>
            <li class="goods-info">
              <ul class="info-class">
                ...
                <li class="info_c">
                  <span class="price"
                        v-show="!good.has_option"><small>¥</small>{{ good.price }}</span>
                  <span class="price"
                        v-show="good.has_option"><small>¥</small>{{ good.max_price }}-{{
                      good.min_price
                      }}</span>
                  <div class="select"
                       v-if="good.has_option != 1">
                    ...
                    <i @click.stop="numberRight(good.id)"
                       class="iconfont icon-life-game-plus"></i>
                  </div>
                  <div class="select optionClass"
                       v-if="good.has_option == 1">
                    <p @click.stop="addGood(good)">可选规格</p><i v-if="!fun.isTextEmpty(goodsCartsTotal[good.id])">{{[good.id].total}}</i>
                  </div>
                </li>
              </ul>
            </li>
    ...
    list: function(newVal, oldVal) {
      if (!this.fun.isTextEmpty(newVal)) {
        ...
        if (
          ...
          !this.fun.isTextEmpty(newVal[0].has_many_children)
        ) {
          this.threeInfo = [];
          this.threeId = null;
