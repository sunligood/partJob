  ...
  <div id="orderList">
    <van-checkbox-group v-model="checkList" @change="multiplePayChange">
      <div
        ...
        :class="
          orderType == 'lease_toy' && index == datasource.length - 1
            ? 'Margintop shop'
            : 'shop'
        "
        :key="index"
      >
        ...
        <router-link
          :to="
            fun.getUrl(
              detailUrl == 'HotelOrderDetail' ? 'HotelOrderDetail' : detailUrl,
              { order_id: order.id, orderType: orderType }
            )
          "
              ...
              <div class="price">
                <div class="money">
                  ...
                  {{
                    orderType == "lease_toy"
                      ? good.price /
                        order.has_one_lease_toy_order.return_days /
                        good.total
                      : good.price
                  }}
                  <span v-if="order.plugin_id == 40 ? true : false">/天</span>
                </div>
                <div v-if="detailUrl != 'HotelOrderDetail'">
                  ×{{ good.total }}
                </div>
                <div v-if="detailUrl == 'HotelOrderDetail'">
                  ×{{
                    getDays(
                      order.hotel_order.enter_at,
                      order.hotel_order.leave_at
                    )
                  }}晚{{ good.total }}间
                </div>
              </div>
    ...
    >
      ...
      <mt-navbar v-model="activeName">
        <mt-tab-item id="first">快递配送</mt-tab-item>
        <mt-tab-item id="second" v-if="isShowDri">司机配送</mt-tab-item>
      </mt-navbar>
      ...
      <div class="store_foot">
        <van-button type="default" class="btn_a" @click="sendCancel"
          >取 消</van-button
        >
        <van-button type="primary" class="btn_b" @click="sendGoogs"
          >确 定</van-button
        >
      </div>
    </van-dialog>
    <!-- <van-dialog @touchmove.native="stopmove"
                @mousemove.native="stopmove"
      ...
      <div class="store_foot">
        <van-button type="default"
                    class="btn_a"
                    @click="driverSendCancel">取 消</van-button>
        <van-button type="primary"
                    class="btn_b"
                    @click="driverSendGoogs">确 定</van-button>
      </div>
    </van-dialog> -->
    ...
    <van-popup v-model="showWriteOff" position="bottom" :overlay="true">
      <img
        :src="writeOffQR"
        style="width:11.25rem;height:11.25rem;margin:2rem 0 1rem 0;"
      />
      <div
        style="font-size:18px;font-weight:bold;margin-bottom: 1rem;color:#f15353"
      >
        请核销员扫码
      </div>
    </van-popup>
...
export default {
  ...
  methods: {
    ...
    operation(btn, order) {
      ...
      } else if (btn.value === "delivery_station_pay") {
        //配送站的确认支付
        this.orderPayId = order.id;
        this.payMoney(order, btn);
      } else if (btn.value === "delivery_station_accept_order") {
        //配送站的确认接单
        this.orderPayId = order.id;
        this.orderReceiving(order, btn);
      } else if (btn.value === "delivery_station_send") {
        //配送站的确认配送
        this.orderPayId = order.id;
        this.confirmDelivery(order, btn);
      } else if (btn.value === "delivery_station_cancel_send") {
        //配送站的取消配送
        this.orderPayId = order.id;
        this.cancelDelivery(order, btn);
      } else if (btn.value === "delivery_station_complete") {
        //配送站的确认收货
        this.orderPayId = order.id;
        this.collectGoods(order, btn);
      } else if (btn.value === "delivery_station_close") {
        //配送站的关闭订单
        this.orderPayId = order.id;
        this.closeOrderPay(order, btn);
      } else if (
      ...
      } else if (btn.value == "sbc_schedule") {
        // 分期购车
        this.$router.push(
          this.fun.getUrl("ApplicationProgress", { id: order.id })
        );
      }
    },
    //司机配送确认发货
    ...
    orderReceiving(order, btn) {
      MessageBox({
        ...
        if (action == "confirm") {
          ...
          let urls =
            "plugin.delivery-station.frontend.order.operation.accept-order";
          $http.get(urls, json).then(
            function(response) {
              if (response.result == 1) {
                that.$emit("ConfrimOrderNotification", "pay");
    ...
    cancelDelivery() {
      MessageBox({
        ...
        if (action == "confirm") {
          ...
          let urls =
            "plugin.delivery-station.frontend.order.operation.cancel-send";
          $http.get(urls, json).then(
            function(response) {
              if (response.result == 1) {
                that.$emit("ConfrimOrderNotification", "pay");
    ...
    getCompany() {
      ...
      if (this.orderType == "supplier") {
        ...
        var urls = "plugin.service-station.frontend.order-list.express";
      } else {
    ...
    sendGoogs() {
      ...
      if (this.orderType == "supplier") {
        ...
        $http
          .post(
            "plugin.service-station.frontend.order.order-operation.send",
            json
          )
          .then(
            function(response) {
