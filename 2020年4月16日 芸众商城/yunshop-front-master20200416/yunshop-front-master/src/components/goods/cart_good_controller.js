...
const SUBMIT_CAR_BUY = "buyCar"; //立即购买
var submitActionTag = "";
...
export default {
  props: ["goods_id", "goods_type", "goods_info", "touchClose"],
  data() {
    return {
      questionTitle: [],
      selected: "1",
      ...
      //分期购车
      staging_buy_car_goods: {},
      help_center_enabled: 0,
      package_deliver_enable: 0,
  ...
  methods: {
    ...
    getGoodsPage(data) {
      ...
      this.help_center_enabled = data.pluginEnabled.help_center_enabled;
      this.package_deliver_enable = data.pluginEnabled.help_center_enabled;
      if (this.help_center_enabled == 1) {
        //获取帮助中心数据
        this.getQuestion();
      }
      this.showGood = true;
    ...
    getData(data) {
      ...
      if (this.goodsInfo.staging_buy_car_goods) {
        this.staging_buy_car_goods = this.goodsInfo.staging_buy_car_goods;
      }
      ...
      $http.get("member.member.isValidatePageGoods", {}).then(
        response => {
          if (response.result === 1) {
            ...
            submitActionTag = SUBMIT_CAR_BUY;
            //判断是否有规格
    ...
    showDefaultActivity(val) {
      if (val.first_strip_key == "coupon") {
        ...
        this.firstActivityCon =
          "本商品满￥" + val.ed_full + "立减￥" + val.ed_reduction + "";
      } else if (
    ...
    getLocation() {
      ...
      if (!myLocation.point) {
        ...
        function onComplete(obj) {
          ...
          myLocation = pos;
          that.getList(pos.city, pos.point.lat, pos.point.lng, "");
        }
    ...
    getList(city, lat, lng, kwd) {
      ...
      $http.get("plugin.package-deliver.frontend.deliver.getList", json).then(
        function(response) {
          if (response.result === 1) {
            that.info = response.data.list.data.slice(0, 2);
            that.deliverName = response.data.name?response.data.name:'自提点';
          } else {
            Toast(response.msg);
          }
        },
    ...
    gotoCommentList() {
      this.$router.push(
        this.fun.getUrl(
          "SelfCarry_info",
          { goodsId: goods_id, tag: "goodsDetail" },
          {}
        )
      );
    },
    ...
    handleTouchend(e) {
      ...
      if (last_distance > 0) {
        ...
        last_distance = 0;
      } else if (last_distance < -0 && last_distance > -(0 + 100)) {
        this.isPullUp = false;
        last_distance = -0;
      } else if (last_distance <= -(0 + 100)) {
        //切换到下一个页面
