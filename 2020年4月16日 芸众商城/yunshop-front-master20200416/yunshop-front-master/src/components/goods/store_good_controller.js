...
export default {
  props: ["goods_id", "goods_type", "touchClose", "goods_info_store", "showDis", "show8"],
  data() {
    return {
      goods_info: {},
      openVip: "2",
  ...
  methods: {
    ...
    setGoodsSpecsChangeInfo1(id) {
      ...
      if (specsManage.length === this.goodsInfo.has_many_specs.length) {
        ...
        for (let i = 0; i < this.goodsInfo.has_many_options.length; i++) {
          if (
            goodsSpecs ===
            this.setGoodsSpecsBySort1(this.goodsInfo.has_many_options[i].specs)
          ) {
    ...
    getGoodsPage(data) {
      ...
      // if(response.data.wx_js_sdk_config.config){
    ...
    getData(data) {
      ...
      this.goodsInfo = data;
      this.salesCount =
        this.goodsInfo.show_sales + this.goodsInfo.virtual_sales;
    ...
    postShow() {
      ...
      $http
        .get(
          ...
          { id: this.$route.params.id, storeid: this.store_id },
          "加载中"
        )
        .then(response => {
          toastPoster.close();
    ...
    goToAdress() {
      ...
      if (this.fun.isWeiXin()) {
        ...
        window.location.href = `https://m.amap.com/navi/?dest=${point.lng},${point.lat}&destName=${this.o2oName}&key=43de56d8e65fe042493541213d9ad7b0`;
      }
