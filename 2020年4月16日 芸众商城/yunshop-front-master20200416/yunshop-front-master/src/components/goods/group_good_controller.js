...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      $http
        .get(
          "plugin.fight-groups.frontend.controllers.fight-groups-goods.get-goods",
          json,
          "加载中"
        )
        .then(
    ...
    setGroupInfo(_id) {
      ...
      for (let i = 0; i < this.gooddatas.has_many_options.length; i++) {
        ...
        if (_optionSpecs == optionsId) {
          ...
          optionsMaxCount = this.gooddatas.has_many_options[i].buy_limit; //当前组合的购买限制数, 为0 时为无限制购买
      ...
      if (!this.fun.isTextEmpty(level_id)) {
        if (this.goodsInfo.has_option == 1) {
          ...
          if (this.fun.isTextEmpty(optionsId)) {
            ...
            var selectGroupLever = this.selectGroupInfo.filter(function(v) {
    ...
    addGoods() {
      ...
      if (optionsMaxCount != 0 && this.goodsCount > optionsMaxCount) {
        Toast("数量超出范围");
        console.log("数量超出范围max=" + this.goodsCount);
        return;
      }
      this.goodsCount++;
    },
